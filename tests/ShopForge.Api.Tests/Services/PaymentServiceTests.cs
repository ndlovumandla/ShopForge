...
{
    [Theory]
    [InlineData("4111111111111111", true)]   // Valid Visa (Luhn passes)
    [InlineData("5500005555555559", true)]   // Valid Mastercard (Luhn passes)
    [InlineData("4111111111111112", false)]  // Invalid (Luhn fails)
    [InlineData("1234567890123456", false)]  // Invalid (Luhn fails)
    [InlineData("", false)]                  // Empty
    public void LuhnCheck_ReturnsExpected(string cardNumber, bool expected)
    {
        var result = LuhnValidate(cardNumber);
    ...
    // Replicated Luhn algorithm (same as in PaymentService)
